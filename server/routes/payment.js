...
router.post("/payment", verifyToken, (req, res) => {
  let totalPrice = Math.round(req.body.totalPrice * 100); //STRIPE rule require amt * 100
  stripe.customers
    .create({
      email: req.decoded.email, //first create user with email
    })
    .then((customer) => {
      return stripe.customers.createSource(customer.id, {
        source: "tok_visa", //create customer source of payment (Credit/Debit)
      });
    })
    .then((source) => {
      return stripe.charges.create({
        //charge customer amount if credit card valid
        amount: totalPrice,
        currency: "usd",
    ...
    .then(async (charge) => {
      //create new Order object and fill up details
      let order = new Order();
